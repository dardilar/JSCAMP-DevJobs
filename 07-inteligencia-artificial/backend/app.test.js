...
// Después de todos los test, se ejecuta para cerrar el servidor
after(async () => {
  return new Promise((resolve, reject) => {
    server.close((err) => {
...
describe('GET /jobs', () => {
  ...
  test('Debe filtrar trabajos por tecnología', async () => {
    ...
    const json = await response.json();
    console.log(json);
    assert.ok(json.data.every(job => job.data.technology.includes(tech)), 'Todos los trabajos deben incluir la tecnologia');
  });
});